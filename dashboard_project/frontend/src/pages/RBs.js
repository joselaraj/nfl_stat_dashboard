        ...
        <table className="rbs-table">
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort('carries')} className="sortable">CARRIES{arrow('carries')}</th>
              <th onClick={() => handleSort('rushing_yards')} className="sortable">YDS{arrow('rushing_yards')}</th>
              <th onClick={() => handleSort('rushing_tds')} className="sortable">TDS{arrow('rushing_tds')}</th>
              <th onClick={() => handleSort('receptions')} className="sortable">REC{arrow('receptions')}</th>
              <th onClick={() => handleSort('receiving_yards')} className="sortable">REC YDS{arrow('receiving_yards')}</th>
              <th onClick={() => handleSort('receiving_tds')} className="sortable">REC TD{arrow('receiving_tds')}</th>
            ...
            {paginatedPlayers.map((p, i) => (
              ...
              <td style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                />
                  ...
                  onClick={() => navigate(`/rbs/${encodeURIComponent(p.player_id)}/detail`)}
                >
                ...
                <td className="highlight">{p.rushing_tds}</td>
                <td>{p.receptions}</td>
