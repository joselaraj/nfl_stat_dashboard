...
const pct = (made, att) => att > 0 ? parseFloat(((made / att) * 100).toFixed(1)) : 0;
...
function KickerDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .get(`/api/k/${encodeURIComponent(player_id)}/detail/`)
    ...
    <div className="kickers-container">
      <div className="qbs-header">
        ...
        <div className="player-identity">
          {headshot_url && (
            <img src={headshot_url} alt={player_display_name} className="player-headshot" onError={(e) => { e.currentTarget.style.display = "none"; }} />
          )}
          <div>
          ...
          <div className="chart-card chart-card--wide">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={chartData}>
                ...
                <ReferenceLine y={85} stroke={C.muted} strokeDasharray="4 4" label={{ value: "starter threshold 85%", fill: C.muted, fontSize: 11, position: "insideTopRight" }} />
                <ReferenceLine y={90} stroke={C.made} strokeDasharray="4 4" label={{ value: "elite 90%", fill: C.made, fontSize: 11, position: "insideTopRight" }} />
                <Line type="monotone" dataKey="fg_pct" name="FG %" stroke={C.made} strokeWidth={2.5} dot={({ cx, cy, payload }) => (
                  <circle key={payload.season} cx={cx} cy={cy} r={5} fill={payload.fg_pct >= 90 ? C.made : payload.fg_pct >= 85 ? C.missed : C.blocked} stroke="none" />
                )} activeDot={{ r: 7 }} />
              </LineChart>
            </ResponsiveContainer>
          ...
          <div className="chart-card" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={pieData} barCategoryGap="30%">
                ...
                <YAxis tick={{ fontSize: 11, fill: C.muted }} allowDecimals={false} />
                <Tooltip content={<ChartTooltip />} />
                <Bar dataKey="value" name="FGs" radius={[3,3,0,0]}>
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={chartData}>
                ...
                <Tooltip content={<ChartTooltip />} />
                <ReferenceLine y={95} stroke={C.muted} strokeDasharray="4 4" label={{ value: "avg 95%", fill: C.muted, fontSize: 11 }} />
                <Line type="monotone" dataKey="pat_pct" name="PAT %" stroke={C.pat} strokeWidth={2.5} dot={{ r: 4, fill: C.pat }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={chartData} barCategoryGap="35%">
                ...
                <Tooltip content={<ChartTooltip />} />
                <ReferenceLine y={50} stroke={C.muted} strokeDasharray="4 4" label={{ value: "50 yds", fill: C.muted, fontSize: 11 }} />
          ...
          <div className="chart-card">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={chartData}>
                ...
                <Tooltip content={<ChartTooltip />} />
                <Line type="monotone" dataKey="fpts_per_game" name="Fant pts/game" stroke={C.missed} strokeWidth={2.5} dot={{ r: 4, fill: C.missed }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              <tbody>
                {chartData.map((s, i) => (
                  <tr key={s.season} className={i % 2 === 0 ? "row-even" : "row-odd"}>
                    ...
                    <td>{s.fg_made}</td>
                    <td>{s.fg_att}</td>
                    <td style={{ color: s.fg_pct >= 90 ? C.made : s.fg_pct < 85 ? C.blocked : "inherit" }}>{s.fg_pct}%</td>
                    <td>{s.fg_long} yds</td>
                    <td>{s.pat_made}</td>
                    <td>{s.pat_att}</td>
                    <td style={{ color: s.pat_pct < 95 ? C.blocked : "inherit" }}>{s.pat_pct}%</td>
                    <td style={{ color: s.gwfg_made > 0 ? C.gwfg : "inherit" }}>{s.gwfg_made > 0 ? `${s.gwfg_made} (${s.gwfg_distance} yds)` : "—"}</td>
            ...
            <table className="qbs-table">
              ...
              <tbody>
                <tr className="row-odd">
                  ...
                  <td>{career.fg_att}</td>
                  <td style={{ color: careerFgPct >= 90 ? C.made : careerFgPct < 85 ? C.blocked : "inherit" }}>{careerFgPct}%</td>
                  <td>{career.pat_made}</td>
                  <td>{career.pat_att}</td>
                  <td style={{ color: careerPatPct < 95 ? C.blocked : "inherit" }}>{careerPatPct}%</td>
                  <td style={{ color: career.gwfg_made > 0 ? C.gwfg : "inherit" }}>{career.gwfg_made}</td>
