...
from Players.models import RunningBacks

class Command(BaseCommand):
    help = 'Load RB season stats into the database'

    def handle(self, *args, **kwargs):
        self.stdout.write("Fetching RB data...")

        years = [2021, 2022, 2023, 2024, 2025]
        df = nfl.load_player_stats(years)

        rbs = df.filter(pl.col('position_group') == 'RB')
        ...
        count = 0
        for row in rbs_season.to_dicts():
            RunningBacks.objects.update_or_create(
        ...
        self.stdout.write(self.style.SUCCESS(f"✅ Loaded {count} RB records."))