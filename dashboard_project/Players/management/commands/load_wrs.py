...
from Players.models import WideReceivers

class Command(BaseCommand):
    help = 'Load WR season stats into the database'

    def handle(self, *args, **kwargs):
        self.stdout.write("Fetching WR data...")

        years = [2021, 2022, 2023, 2024, 2025]
        df = nfl.load_player_stats(years)

        wrs = df.filter(pl.col('position_group') == 'WR')
        ...
        count = 0
        for row in wrs_season.to_dicts():
            WideReceivers.objects.update_or_create(
        ...
        self.stdout.write(self.style.SUCCESS(f"✅ Loaded {count} WR records."))